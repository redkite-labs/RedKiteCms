...
$(document).ready(function()
{
    ...
    $('.rkcms-slot').each(function(){
        ...
        var slotName = element.attr("id");
        var slot = new SlotModel(blocks, slotName, element.attr("data-outdated"), element.attr("data-contributions"));
    ...
    $('#rkcms-control-panel-btn')
        ...
        .on('shown.bs.popover', function () {
            ...
            var model = new ButtonEditModel(status, moveMode);
    ...
    $('#rkcms-pages-editor-table').each(function(){
        ...
        var pages = new PageModel(pageValues);
        ko.applyBindings(pages, document.getElementById('rkcms-pages-editor-table'));
    });

    $(document).on("rkcms.event.opening_panel", function(event, model, koEvent){
        // Closes the active slot's editor panel if opened
        var activeSlot = $(document).data('rkcms-ko-active-slot');
        if (null != activeSlot) {
            activeSlot.closePanel(activeSlot, koEvent);
        }
        
        // Closes the active block's editor panel if opened
        var activeBlock = $(document).data('rkcms-active-model');
        if (null != activeBlock) {
            if (activeBlock != model) {
                activeBlock.closePanel(activeBlock, koEvent);
            }

            if (activeBlock.historyPanel()) {
                activeBlock.historyBack();
            }
        }

        // Highlights the element
        $(koEvent.target).highlight('activate');
    });
    
    $(document).on("rkcms.event.closing_panel", function(event, view, koEvent){
        $(koEvent.target).highlight('close');
    });

    $(document).on("rkcms.event.block_removed", function(event, view, koEvent){
        $(koEvent.target).highlight('close');
    });
...
(function($){
    ...
    function _moveBlockToSameSlot(ui, sortable)
    {
        ...
        var url = frontcontroller + '/backend/en/moveBlock';
        var ajaxData = {
...
var SeoModel = function (seo)
{
    ...
    self.permalink = ko.observable(seoData.permalink);
    ...
    self.language = seoData.language;
    self.changedPermalinks = seoData.changed_permalinks;
    self.togglePanel = ko.observable(false);

    self.toggle = function()
    {
        self.togglePanel(!self.togglePanel());
    };

    self.edit = function(seo, event)
    {
        var url = frontcontroller + '/backend/seo/edit';
        var data = {
            'page-name': page,
            'seo-data': prepareSeoData()
        };
    ...
    self.approve = function(seo)
    {
        var url = frontcontroller + '/backend/seo/approve';
        var data = {
            'pageName': page,
            'seo-data': prepareSeoData()
        };
...
var Block = function (params)
{
    ...
    self.togglePanel = ko.observable(false);
    self.attributesPanel = ko.observable(true);
    ...
    self.blockToJson = function()
    {
        alertDialog(redkitecmsDomain.frontend_missing_block_to_json_method_implementation, null, 'danger');
    }.bind(self);

    self.highlight = function (view, event)
    {
        if ( ! self.parent.active) {
            return true;
        }

        $(event.target).highlight('highlight');
    }.bind(self);

    self.hide = function (view, event)
    {
        $(event.target).highlight('hide');
    }.bind(self);

    self.toggle = function(value)
    {
        self.togglePanel(value);
    }.bind(self);

    self.edit  = function()
    {
        $(document).trigger("rkcms.event.editing_block", [ self ]);
        self.doEdit(self);
    }.bind(self);

    self.startBlockEditing = function(view, event)
    {
        if ( ! self.parent.active) {
            return true;
        }

        $(document).trigger("rkcms.event.opening_panel", [ self, event ]);
        self.doStartBlockEditing(self, event);
        $(document).trigger("rkcms.event.panel_opened", [ self, event ]);
    }.bind(self);

    self.resize  = function()
    {
        var element = $(document).data('rkcms-active-element');
        if (element != null) {
            $(element).highlight('resize');
        }
    ...
    {
        _closePanels();
        self.availableBlocksPanel(true);
    }.bind(self);

    self.openHistoryPanel = function()
    {
        _closePanels();
        self.historyPanel(true);
    }.bind(self);

    self.remove = function(view, event)
    {
        var message = redkitecmsDomain.frontend_confirm_block_remove;
        confirmDialog(message, function(){
            $(document).trigger("rkcms.event.removing_block", [ self, event ]);
            self.doRemove(view, event);
        });
    }.bind(self);

    self.dockTop = function (view, event)
    {
        self.parent.dockTop(view, event);
        self.dockStatus('top');
    }.bind(self);

    self.dockBottom = function (view, event)
    {
        self.parent.dockBottom(view, event);
        self.dockStatus('bottom');
    }.bind(self);

    self.closePanel = function(view, event)
    {
        $(document).trigger("rkcms.event.closing_panel", [ self, event ]);
        self.doClosePanel(view, event);
        $(document).trigger("rkcms.event.panel_closed", [ self, event ]);
    }.bind(self);

    // ADD PANEL BUTTONS
    self.selectBlock = function (view, event)
    {
        self.parent.selectBlock(view, event);
    }.bind(self);

    self.addTop = function(view, event)
    {
        $(document).trigger("rkcms.event.adding_block", [ self ]);
        self.doAdd(view, event, 'top', self.transactionAddTop);
    }.bind(self);

    self.addBottom = function(view, event)
    {
        $(document).trigger("rkcms.event.adding_block", [ self ]);
        self.doAdd(view, event, 'bottom', self.transactionAddBottom);
    }.bind(self);

    self.back = function()
    {
        _closePanels();
        self.attributesPanel(true);
    }.bind(self);

    // RESTORATION PANEL BUTTONS
    self.restore = function(archivedBlock)
    {
        alertDialog(redkitecmsDomain.frontend_missing_restore_method_implementation, null, 'danger');
    }.bind(self);

    self.undoRestoration = function (view, event)
    {
        if (self.cloneBlock == null) {
            return false;
        }

        self.cloneBlock.restore(self.cloneBlock.block);
        self.resize();
        $('.rkcms-selected-block').removeClass('rkcms-selected-block');
        self.activeListItemBlock.isDirty = false;
    }.bind(self);

    self.confirmRestoration = function (view, event)
    {
        $(document).trigger("rkcms.event.restoring_block", [ self ]);
        self.doRestore(self);
    }.bind(self);

    self.historyBack = function(view, event)
    {
        if (self.activeListItemBlock != null && self.activeListItemBlock.isDirty) {
            self.undoRestoration(view, event);
        }
        self.back();
    }.bind(self);
    ...
    {
        self.attributesPanel(false);
        self.availableBlocksPanel(false);
        self.historyPanel(false);
    }

    self.initHistory = function(parent, values)
    {
        if (null == values) {
           return;
        }
        ...
        ko.utils.arrayForEach(values, function(block){
            ...
            self.history.push(new ArchiveBlock(parent, block));
        });
    };

    // Initialization
    // ==============
    self.parent.dock(self);
    self.initHistory(self, self.block.history);
};

Block.prototype.doStartBlockEditing = function(view, event, togglePanel)
{
    $(document).data('rkcms-active-model', view);
    $(document).data('rkcms-active-element', event.target);
    if (null == togglePanel) {
        togglePanel = true;
    }

    if (togglePanel) {
        view.toggle( ! view.togglePanel());
    }
    view.parent.dock(view);
};

Block.prototype.doAdd = function(view, event, direction, transactionIcon, type, position)
{
    transactionIcon(true);
    var block = view.block;
    var blocks = view.parent.blocks;
...
Block.prototype.doEdit = function(view, event)
{
    ...
    var data = {
        ...
        'language': language,
        'country': country,
        'data': view.blockToJson()
    };
    executeAjax(url, data,
        function(response)
        {
            view.initHistory(view, response.history);
            $(document).trigger("rkcms.event.block_edited", [ view, response ]);
        }
    );
};

Block.prototype.doRestore = function(view, event)
{
    if (view.activeListItemBlock == null) {
        return;
    }
    ...
    var data = {
        ...
        'type': view.type,
        'slot': view.slotName,
        'archiveFile': view.activeListItemBlock.historyName,
        'page':  page,
        'language': language,
        'country': country,
        'data': view.blockToJson()
    ...
    executeAjax(url, data,
        function(response)
        {
            ...
            view.initHistory(view, response);
            $(document).trigger("rkcms.event.block_restored", [ view ]);
        }
    );
};

Block.prototype.doRemove = function(view, event)
{
    //view.transactionRemove(true);
    var index = view.parent.blocks.indexOf(view.block);
    view.parent.blocks.remove(view.block);
    ...
    executeAjax(url, data,
        function(response)
        {
            ...
            $(document).trigger("rkcms.event.block_removed", [ view, event ]);
        },
        null,
        function()
        {
            view.transactionRemove(false);
        }
    );
};

Block.prototype.doClosePanel = function(view)
{
    view.togglePanel(false);
    $(document).data('rkcms-active-model', null);
    $(document).data('rkcms-active-element', null);
};
var ButtonEditModel = function (status, moveMode) {
    var self = this;    
    if (null == status) {
        status = 'off';
    }
    ...
    self.startEdit = function (view, event)
    {
        if (self.moveMode() == 'on') {
            stopMoving();
        }
    ...
    self.stopEdit = function (view, event)
    {
        stopEditor();
    };

    self.stopMove = function (view, event)
    {
        stopMoving();
    };

    self.startMove = function (view, event)
    {
        stopEditor();
    ...
    self.savePage = function (view, event)
    {
        if (self.isTheme() == 'on'){
    ...
    self.saveAllPages = function (view, event)
    {
        if (self.isTheme() == 'on'){
        ...
        confirmDialog(message, function(){
            var url = frontcontroller + '/backend/page/save-all';
            var data = {
                'page':  page,
                'language': language,
                'country': country
    ...
    self.saveTheme = function (view, event)
    {
    ...
    self.seoPanel = function (view, event)
    {
        var seoModel = ko.dataFor(document.getElementById('rkcms-seo'));
        var element = $(event.target).parent();
        toggleButtonPressed(element, seoModel.togglePanel());

        seoModel.toggle();
    };

    self.pagePublished = function (view, event)
    {
        var message = redkitecmsDomain.frontend_confirm_publish_page;
        var url = frontcontroller + '/backend/seo/publish';
        if (pagePublished) {
            message = redkitecmsDomain.frontend_confirm_hide_page;
            url =  frontcontroller + '/backend/seo/hide';
        }
        ...
        confirmDialog(message, function(){
            ...
            executeAjax(url, data, function(){
                toggleButtonPressed(element, pagePublished);
    ...
    toggleButtonPressed = function(element, pressed)
    {
        if(!pressed) {
    ...
    stopEditor = function()
    {
        ...
        var activeElement = $(document).data('rkcms-active-element');
        if (null != activeElement){
            $(activeElement).highlight('close');
        }

        var activeModel = $(document).data('rkcms-active-model');
        if (null != activeModel){
            activeModel.doClosePanel(activeModel);
        }
    ...
    updateMoveMode = function(status)
    {
    ...
    toggleButtonPressed($('#rkcms-page-published'), !pagePublished);
};
var SlotModel = function (blocks, slotName, outdated, contributions)
{
    var self = this;
    ...
    self.hasContributions = ko.computed(function(view, event) {
        return self.contributions().length > 0;
    });

    self.initContributions = function(parent, values)
    {
        if (null == values) {
            return;
        }
        ...
        ko.utils.arrayForEach(values, function(block){
            self.contributions.push(new ContributionBlock(parent, block));
        });
    };
    
    self.openContributionsPanel = function(view, event)
    {
        if ($(document).data('rkcms.active-slot') != undefined){
            var activeContributionBlock = $(document).data('rkcms.active-contribution-block');
            if (activeContributionBlock != undefined) {
                activeContributionBlock.togglePreview();
            }
        }
        $(document).data('rkcms.active-slot', self);
    };

    self.dock = function(element){
        var dockStatus = $('body').data('rkcms.dock-status');
        if (null == dockStatus) {
            element.dockBottom();
    ...
    self.dockTop = function (view, event)
    {
        ...
        $('.rkcms-block-editor').addClass('rkcms-dock-top').removeClass('rkcms-dock-bottom');
        $('body').data('rkcms.dock-status', dockStatus);
    };

    self.dockBottom = function (view, event)
    {
        var dockStatus = 'bottom';
        self.dockStatus(dockStatus);
        $('.rkcms-block-editor').addClass('rkcms-dock-bottom').removeClass('rkcms-dock-top');
        $('body').data('rkcms.dock-status', dockStatus);
    };

    self.activate = function (view, event)
    {
        self.active = true;
    };

    self.deactivate = function (view, event)
    {
        self.active = false;
    ...
    self.showPanel = function(view, event)
    {
        if ( ! self.active) {
            return true;
        }

        var activeBlock = retrieveActiveElementModel();
        if (null != activeBlock) {
            activeBlock.closePanel(view, event);
        }
            
        var activeSlot = $(document).data('rkcms-ko-active-slot');
        if (null != activeSlot) {            
            activeSlot.closePanel(view, event);
        }
        $(document).data('rkcms-ko-active-slot', self);
        self.togglePanel(true);
        $(event.target).highlight('activate');
    ...
    {
        var activeBlock;
        var activeBlockElement = $(document).data('rkcms-ko-active-element');
        if (activeBlockElement != null) {
            activeBlock = ko.dataFor(activeBlockElement);
        }
        if (activeBlock != null) {
            window.setTimeout(function(){
                Holder.run();
    ...
    self.addBlock = function(view, event)
    {
        ...
        executeAjax(url, data,
            function(response)
            {
                response.slot_name = self.slotName;
                blocks.splice(position, 0, response);
            },
            null,
            function()
    ...
    {
        self.togglePanel(false);
        $(event.target).highlight('close');
        $('.rkcms-selected-block').removeClass('rkcms-selected-block');
    }.bind(self);

    retrieveActiveElementModel = function()
    {
        var activeBlock = null;
        var activeBlockElement = $(document).data('rkcms-active-element');
        if (activeBlockElement != null) {
            activeBlock = ko.dataFor(activeBlockElement);
        }
    ...
    // ==============
    self.dock(self);
    self.initContributions(self, ko.utils.parseJson(contributions));
};
var PageModel = function (pages)
{
    ...
    {
        var newValue = ko.utils.parseJson(decodeURIComponent($('#rkcms-add-page-button').attr('data-new-page')));
        if (pageExists(newValue, self.pages()))
        {
        ...
        var url = frontcontroller + '/backend/page/add';
        var data = {
            'data': newValue
        };
    ...
    self.editPage = function(page)
    {
        ...
        console.log(pageClone.approved());
    ...
    self.remove = function(page)
    {
        ...
        confirmDialog(message, function(){
            ...
            var url = frontcontroller + '/backend/page/remove';
            var data = {
                'page-data': page
            };
    ...
    self.approve = function(page)
    {
        var url = frontcontroller + '/backend/page/approve';
        var data = {
            'pageName': page.currentName
        };

        executeAjax(url, data,
            function(response) {
                page.toBeApproved(false);
            }
        );
    };
    ...
    {
        // The page name is saved as oldName because it is needed to save the page when user changes the page name
        page.currentName = page.name;
        page.showDetails = ko.observable(false);
        page.name = ko.observable(page.name);
        /*
        page.approved = ko.observable(page.approved === '1');
        //page.toBeApproved = ko.observable(page.toBeApproved === 'true');
        ko.utils.arrayForEach(page.seo, function(seo){
            seo.permalink = ko.observable(seo.permalink);
            //seo.toBeApproved = ko.observable(seo.toBeApproved === 'true');
        });*/
    }console.log(pages);
...
{
    var self = this;
    PagesSelector.call(self, params);

    var src = (self.block.src == "" || self.block.src == self.block.data_src) ? self.block.data_src : self.block.src;

    self.src = ko.observable(src);
    self.title = ko.observable(self.block.title);
    self.alt = ko.observable(self.block.alt);
    self.url = ko.observable(self.block.href);
    self.linkClass = ko.observable(self.block.link_class);
    self.file = ko.observable(self.block.file);
    self.data_src = ko.observable(self.block.data_src);
    self.hasLink = ko.computed(function(){ return self.url() != "" });
    ...
    self.src.subscribe(function(value){
    ...
    self.src.subscribe(function(value){
        if (value.match(/holder/)) {
            return;
        }

        var image = basename(self.src());
        self.data_src('');
        if (!self.title()) {
            self.title(image)
        }
    ...
    self.title.subscribe(RunHolder);
    self.alt.subscribe(RunHolder);
    self.url.subscribe(RunHolder);
    self.linkClass.subscribe(RunHolder);
    self.file.subscribe(RunHolder);
    self.cssClass.subscribe(RunHolder);

    self.blockToJson = function() {
        var block = self.block;
        block.src = self.src();
        block.data_src = self.data_src();
        block.title = self.title();
        block.alt = self.alt();
        block.cssClass = self.cssClass();
        block.href = self.url();
        block.link_class = self.linkClass();
        block.file = self.file();
    ...
    self.restore = function(archivedBlock)
    {
        self.src(archivedBlock.src);
        self.title(archivedBlock.title);
        self.alt(archivedBlock.alt);
        self.href(archivedBlock.href);
        self.link_class(archivedBlock.link_class);
        self.file(archivedBlock.file);
        self.cssClass(archivedBlock.cssClass);
    };

    self.openImagesSelector  = function(element){
        var url = frontcontroller + '/backend/elfinder/media/connect';
        mediaLibrary(url, function(file, fm){
            element.src(file.url);
            var currentBlock = $(document).data('rkcms-active-element');
            $(currentBlock)
                .css("width", "auto")
                .css("height", "auto")
            ;
    ...
    self.openFilesSelector  = function(element){
        var url = frontcontroller + '/backend/elfinder/files/connect';
        mediaLibrary(url, function(file, fm){
...
Image.prototype = Object.create(PagesSelector.prototype);
...
$(document).on("rkcms.event.block_edited", function(){
    RunHolder();
});
...
;var Text = function (params)
{
    ...
    _init = function (element, obj)
    {
        ...
        tinymce.init({
            ...
            toolbar: "save | styleselect | bold italic | bullist numlist | link image table | source",
            ...
            setup: function(editor) {
                editor.on("focus", function(event)
                {
                    ...
                    $('body').data('rkcms-active-mce-editor', editor);
                    self.isActive = true;
                });
...
Text.prototype.doStartBlockEditing = function(view, event)
{
    Block.prototype.doStartBlockEditing(view, event, false);

    view.highlightable = false;
    _init(view, event.target);
    $(event.target).highlight('close');
    view.toggle(true);
};

Text.prototype.doClosePanel = function(view)
{
    $('body').data('rkcms-active-mce-editor').destroy();
    Block.prototype.doClosePanel(view);
    view.highlightable = true;
    view.isActive = false;
};
...
var Link = function (params)
{
    ...
    self.restore = function(archivedBlock)
    {
        self.url(archivedBlock.url);
        self.value(archivedBlock.value);
        self.cssClass(archivedBlock.cssClass);
    };
};

Link.prototype = Object.create(PagesSelector.prototype);
