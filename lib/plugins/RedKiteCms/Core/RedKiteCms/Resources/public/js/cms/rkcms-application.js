...
function InitSlots()
{
    var slots = localStorage.getItem('rkcms-slots');
    slots= null;
    if(slots == null) {
        slots = [];
        $('.rkcms-slot').each(function(){
            ...
            var blocks = ko.utils.parseJson(decodeURIComponent(element.attr("data-slot")));
            var slotName = element.attr("data-slotname");
            var slot = new SlotModel(blocks, slotName, blockEditorModel);

            slots.push(ko.toJSON({
                'slot' : ko.toJSON(slot)
                //'item' : JSON.stringify($(this)[0])
            }));
            ko.applyBindings(slot, this);
        });
        localStorage.setItem('rkcms-slots', slots);

        return;
    }
    console.log(JSON.parse(slots));/*
    ko.utils.arrayForEach(slots, function(item, x){
        console.log(item);
    });*/
}
