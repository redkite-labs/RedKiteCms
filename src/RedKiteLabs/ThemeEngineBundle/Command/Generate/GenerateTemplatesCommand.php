<?php

namespace AlphaLemon\ThemeEngineBundle\Command\Generate;

use Symfony\Bundle\FrameworkBundle\Command\ContainerAwareCommand;
use Symfony\Component\Console\Input\InputInterface;
use Symfony\Component\Console\Input\InputOption;
use Symfony\Component\Console\Input\InputArgument;
use Symfony\Component\Console\Output\OutputInterface;
use AlphaLemon\ThemeEngineBundle\Core\Generator\AlTemplateGenerator;
use Symfony\Component\DependencyInjection\Container;
use Sensio\Bundle\GeneratorBundle\Command\Helper\DialogHelper;
use AlphaLemon\ThemeEngineBundle\Core\Generator\TemplateParser\AlTemplateParser;
use AlphaLemon\ThemeEngineBundle\Core\Generator\AlSlotsGenerator;

class GenerateTemplatesCommand extends ContainerAwareCommand
{
    protected $templateParser;
    protected $templateGenerator;
    protected $slotsGenerator;

    public function setTemplateParser(AlTemplateParser $templateParser)
    {
        $this->templateParser = $templateParser;
    }

    public function getTemplateParser()
    {
        return $this->templateParser;
    }

    public function setTemplateGenerator(AlTemplateGenerator $templateGenerator)
    {
        $this->templateGenerator = $templateGenerator;
    }

    public function getTemplateGenerator()
    {
        return $this->templateGenerator;
    }

    public function setSlotsGenerator(AlSlotsGenerator $slotsGenerator)
    {
        $this->slotsGenerator = $slotsGenerator;
    }

    public function getSlotsGenerator()
    {
        return $this->slotsGenerator;
    }

    protected function configure()
    {
        $this
            ->setName('alphalemon:generate:templates')
            ->setDescription('Generate the templates config files for the given theme')
            ->setDefinition(array(
                new InputArgument('theme', InputArgument::REQUIRED, 'The name of the theme bundle which gets the template'),
            ));
    }

    /**
     * @see Command
     *
     * @throws \InvalidArgumentException When namespace doesn't end with Bundle
     * @throws \RuntimeException         When bundle can't be executed
     */
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        $themeName = $input->getArgument('theme');
        $kernel = $this->getContainer()->get('kernel');
        $dir = $kernel->locateResource('@' . $themeName);

        if (null === $this->templateParser) {
            $this->templateParser = new AlTemplateParser($dir . 'Resources/views/Theme');
        }

        if (null === $this->templateGenerator) {
            $this->templateGenerator = new AlTemplateGenerator();
        }

        if (null === $this->slotsGenerator) {
            $this->slotsGenerator = new AlSlotsGenerator();
        }

        $extensionFiles = array();
        $templates = $this->templateParser->parse();
        $this->addOption('template-name', '', InputOption::VALUE_NONE, '');
        foreach ($templates as $templateName => $elements) {
            $templateName = basename($templateName, '.html.twig');
            if ($elements['generate_template']) {
                $extensionFiles['templates'][] = $this->writeLoader(Container::underscore($templateName));
                $message = $this->templateGenerator->generateTemplate($dir . 'Resources/config/templates', $themeName, $templateName, $elements['assets']);
                $output->writeln($message);
            }

            $slots = $elements['slots'];
            if (!empty($slots)) {
                $extensionFiles['slots'][] = $this->writeLoader(Container::underscore($templateName));
                $message = $this->slotsGenerator->generateSlots($dir . 'Resources/config/templates/slots', $themeName, $templateName, $slots);

                $output->writeln($message);
            }
        }

        if (isset($extensionFiles['templates']) && isset($extensionFiles['slots'])) {
            $theme = substr($themeName, 0, -6);
            $extensionFile = sprintf('%sDependencyInjection/%sExtension.php', $dir, $theme);
            if (file_exists($extensionFile)) {
                $this->generateExtensionsCode($extensionFile, $theme, $extensionFiles);
            }
        }
    }

    private function generateExtensionsCode($extensionFile, $themeName, array $extensionFiles)
    {
        $extensionCode = $this->writeLoader(Container::underscore($themeName)) . PHP_EOL ;
        $extensionCode .= $this->writeFileLocator('__DIR__ . \'/../Resources/config/templates\'');
        $extensionCode .= implode("", $extensionFiles['templates']) . PHP_EOL;
        $extensionCode .= $this->writeFileLocator('__DIR__ . \'/../Resources/config/templates/slots\'');
        $extensionCode .= implode("", $extensionFiles['slots']);

        $placeholder = '// This code is autogenerated - DON\'T CHANGE IT MANUALLY';
        $extensionCode =
            $placeholder .
            PHP_EOL .
            PHP_EOL .
            $extensionCode .
            PHP_EOL .
            '        ' . $placeholder;

        $contents = file_get_contents($extensionFile);

        $replacedContent = '';
        $escapedPlaceholder = '\/\/ This code is autogenerated \- DON\'T CHANGE IT MANUALLY';
        if (preg_match('/' . $escapedPlaceholder . '/s', $contents, $matches)) {
            $replacedContent = preg_replace('/' . $escapedPlaceholder . '\n(.*?)[\n\s]*' . $escapedPlaceholder . '/s', $extensionCode, $contents);
        }
        else {
            $extensionCode = '$loader->load(\'services.xml\');' . PHP_EOL . PHP_EOL . '        ' . $extensionCode;
            $replacedContent = preg_replace('/\$loader\-\>load\(\'services\.xml\'\);/s', $extensionCode, $contents);
        }

        if ($replacedContent != '') {
            file_put_contents($extensionFile, $replacedContent);
        }
    }

    private function writeFileLocator($path)
    {
        return sprintf('        $loader = new XmlFileLoader($container, new FileLocator(%s));', $path) . PHP_EOL;
    }

    private function writeLoader($file)
    {
        return sprintf('        $loader->load(\'%s.xml\');', $file) . PHP_EOL;
    }
}