...
abstract class AlTwigTemplateWriter
{
    ...
    protected function generateContentsSection($filter = null)
    {
        ...
        foreach ($blocks as $slotName => $slotBlocks) {
            ...
            foreach ($slotBlocks as $block) {
                ...
                if (null !== $blockManager) {
                    ...
                    // @codeCoverageIgnoreStart
                    if ($needsCredits && preg_match('/\<a[^\>]+href="http:\/\/redkite\-labs\.com[^\>]+\>powered by redkite cms\<\/a\>/is', strtolower($content))) {
                        $needsCredits = false;
                    }
                    // @codeCoverageIgnoreEnd
        ...
        if ($needsCredits) {
            ...
            $this->contentsSection .= '  {{ parent() }}' . PHP_EOL. PHP_EOL;
            $this->contentsSection .= '  <div class="al-credits"><a href="http://redkite-labs.com">Powered by RedKiteCms</div>' . PHP_EOL;
