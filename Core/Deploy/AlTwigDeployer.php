...
class AlTwigDeployer extends AlDeployer
{
    ...
    protected function save(AlPageTree $pageTree)
    {
        ...
        foreach ($pageTree->getContents() as $slotName => $contents) {
            if (!in_array($slotName, $slots)) continue;
            
            $htmlContents = array();
            foreach ($contents as $content) {
                $htmlContents[] = $content['HtmlContent'];
                if (!in_array($content['Block']['ClassName'], $apps)) $apps[] = $content['Block']['ClassName'];
            }
        ...
        foreach ($apps as $app) { 
            $param = strtolower($app) . '_required_stylesheets';
            if($this->container->hasParameter($param)) $appsStylesheets = array_merge($appsStylesheets, $this->container->getParameter($param));
            
            $param = strtolower($app) . '_required_javascripts';
            if($this->container->hasParameter($param)) $appsJavascripts = array_merge($appsJavascripts, $this->container->getParameter($param));
        }
        ...
        if($pageTree->getInternalJavascript() != "") $twig .= $this->writeBlock('internal_header_javascripts', '<script>$(document).ready(function(){' . $pageTree->getInternalJavascript() . '});</script>');
        if($pageTree->getInternalStylesheet() != "") $twig .= $this->writeBlock('internal_header_stylesheets', '<style>' . $pageTree->getInternalStylesheet() . '</style>');
    ...
    protected function writeAssetic($sectionName, $assets, $content, $filter = null, $output = null)
    {
        ...
        if(null !== $filter) $section .= " filter=\"$filter\"";
        if(null !== $output) $section .= " output=\"$output\"";
        $block = "  {% $section %}\n";
